...
void env_set_nice(struct Env* e, int nice_value)
{
	...
	e->nice = (nice_value > 20) ? 20 :(nice_value < -20) ? -20 : nice_value;

	if(e->env_status != ENV_NEW)
	{
		int new_periority = fix_trunc(fix_sub(fix_sub(fix_int(num_of_ready_queues - 1), fix_div((e->recent_cpu),fix_int(4))),fix_int(nice_value * 2)));
		int maxi = num_of_ready_queues - 1;

		if(new_periority > maxi)
			new_periority = maxi;
		if(new_periority < 0)
			new_periority = 0;

		e->priority = new_periority;
	}
}

int env_get_recent_cpu(struct Env* e)
{
	cprintf("env_get_recent_cpu\n");
	...
	int roundedNumber = fix_round(fix_scale(e->recent_cpu, 100));
	return roundedNumber;
}
int get_load_average()
{
	cprintf("get_load_average\n");

	//TODO: [PROJECT'23.MS3 - #3] [2] BSD SCHEDULER - get_load_average
	//Your code is here
	//Comment the following line
	//panic("Not implemented yet");
	int roundedNumber = fix_round(fix_scale(load_avg, 100));
	return roundedNumber;
}
uint32 get_number_of_processes()
{
	uint32 ans = 0;
	for (int i = 0 ; i < num_of_ready_queues ; i++)
