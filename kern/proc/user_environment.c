...
void env_free(struct Env *e)
{
	...
	// virtual address: 10 bit {starting from weight 2^22} 10 bit {starting from weight 2^12} 12 bit {starting from weight 2^0}

	// freeing the page tables inside the current page directory:

	uint32* ptr_page_table;
	for(int i = 0; i < 1024; i++)
	{
		struct FrameInfo* frame = get_frame_info(curenv->env_page_directory, (uint32)e->env_page_directory + i * PAGE_SIZE, &ptr_page_table);
		if(frame != NULL)
			unmap_frame(e->env_page_directory, (uint32)e->env_page_directory + i * PAGE_SIZE);
	}

	// freeing the current page directory

	struct FrameInfo* frame = get_frame_info(curenv->env_page_directory, (uint32)e->env_page_directory, &ptr_page_table);
	if(frame != NULL)
		unmap_frame(e->env_page_directory, (uint32)e->env_page_directory);
...
{
//	cprintf("initialize_uheap_dynamic_allocator\n");

	e->start_user = daStart;
	e->hard_limit_user = daLimit;
	uint32 initial_size = 0;
	e->segment_break_user = daStart + initial_size;
	initialize_dynamic_allocator(e->start_user, initial_size);
