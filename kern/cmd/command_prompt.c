...
int process_command(int number_of_arguments, char** arguments)
{
	...
	int number_of_commands = NUM_OF_COMMANDS;
	...
	for(int i = 0 ; i < number_of_commands ; i++) {
		if (strncmp(arguments[0],commands[i].name,command_len) == 0 && strlen(commands[i].name) == command_len )
		{
	...
	if(command_len != 0)
	{
		for(int i = 0 ; i < number_of_commands ; i++)
		{
			...
			{
				if (commands[i].name[j] == arguments[0][index])
					index++;
