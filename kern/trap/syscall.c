...
{
	// parameters validation check the virtual address is the user limit

	if(virtual_address == 0 || virtual_address > USER_LIMIT || virtual_address + size > USER_LIMIT )  //first condition
		{
...
{
//		cprintf("sys_sbrk with increment = %d\n",increment);
		struct Env* env = curenv;
		//the current running Environment to adjust its break limit

		uint32 previous_break = env->segment_break_user;
		if (increment == 0)
		{
//			cprintf("\n env->segment_break_user = %u \n",env->segment_break_user);
			return (void*) env->segment_break_user;
		}

		else if (increment > 0)
		{
			if (ROUNDUP(env->segment_break_user + increment,PAGE_SIZE) > env->hard_limit_user)
			{
				return (void*)-1;
			}

			uint32 new_break = ROUNDUP(env->segment_break_user + increment, PAGE_SIZE);

			uint32 no_pages = (new_break - ROUNDUP(previous_break,PAGE_SIZE) ) / PAGE_SIZE;
			env->segment_break_user = new_break;

			uint32 cur_page = ROUNDUP(previous_break, PAGE_SIZE);

			allocate_user_mem(curenv,cur_page,no_pages * PAGE_SIZE);

			return (void*) previous_break;
		}
		else
		{
//			cprintf("sys_sbrk decrement\n");
			uint32 decrement = increment * (-1);

			if (env->segment_break_user - decrement < env->start_user)
				return (void*)-1;

			uint32 last_allocated_page = ROUNDDOWN(env->segment_break_user , PAGE_SIZE);
			uint32 last_needed_allocated_page = ROUNDDOWN(env->segment_break_user - decrement , PAGE_SIZE);
			uint32 no_pages_dealloc = (last_allocated_page - last_needed_allocated_page) / PAGE_SIZE;

			if((env->segment_break_user - decrement) % PAGE_SIZE == 0 && !(env->segment_break_user % PAGE_SIZE == 0 && (env->segment_break_user - decrement) % PAGE_SIZE == 0))
				no_pages_dealloc++;

			uint32 page_address = ROUNDDOWN(env->segment_break_user , PAGE_SIZE);
			env->segment_break_user -= decrement;

			while(no_pages_dealloc--)
			{
				uint32* ptr_page_table = NULL;

//				int retutn_table = get_page_table(env->env_page_directory, page_address, &ptr_page_table);
//				if(retutn_table == TABLE_NOT_EXIST)
//					create_page_table(env->env_page_directory , page_address);

				pt_set_page_permissions(env->env_page_directory ,page_address, 0x000, PERM_MARKED);

				unmap_frame(env->env_page_directory, page_address);
...
uint32 sys_hard_limit_user ()
{
	return curenv->hard_limit_user;
}
...
//			cprintf("in sys user mem");
	...
	case SYS_Hard_Limit_User:
		 //hard limit in kern
		return (uint32)sys_hard_limit_user();
		break;
